...
import { ArrowRight, CheckCircle2, MapPin, Building2, Award } from "lucide-react";
...
const groupCompanies = [
  {
    ...
    focus: ["Semi-finished thermoplastic sheets, rods & tubes", "Compression-moulded and extruded profiles", "CNC precision machined components", "Brand families: POLYRIB, KAYLON, PAKETAL, PCCLEAR"],
  },
  {
    ...
    focus: ["Thermoplastic machine components & wear parts", "RIPLA wear strips and guide profiles", "CUTRITE cutting and chopping boards", "Vacuum formed and fabricated plastic assemblies"],
  },
  {
    ...
    focus: ["Nationwide distribution & logistics", "Raw polymer material sourcing", "Distribution centre operations (Delhi, Bhiwandi, Ahmedabad)", "Export co-ordination and international trade"],
  },
];
  ...
  return (
    <div className="pt-16">
      ...
      <div className="relative min-h-[40vh] flex items-end overflow-hidden bg-charcoal">
        <img src={heroImage} alt="Polyrib manufacturing facility" className="absolute inset-0 w-full h-full object-cover opacity-40" />
        ...
        <div className="container max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-start">
            <div>
              ...
              <Link to="/contact" className="mt-8 inline-flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary-dark transition-colors duration-200">
                Get in Touch <ArrowRight className="w-4 h-4" />
              </Link>
            </div>

            {/* Facility image + stat strip */}
            <div className="relative">
              <img src={manufacturingImage} alt="Polyrib manufacturing plant" className="w-full h-72 object-cover" />
              <div className="grid grid-cols-2 gap-px bg-divider border border-divider mt-px">
                {[
      ...
      {/* Group Companies */}
      <section ref={ref1} className="fade-up py-16 border-b border-divider">
        <div className="container max-w-7xl mx-auto px-6">
          <p className="section-label mb-3">Group Structure</p>
          <h2 className="font-heading text-2xl text-charcoal mb-2">Three companies, one group</h2>
              ...
              <div key={co.name} className="border border-border hover:border-primary/40 transition-colors duration-200 flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      <span className="text-[10px] font-semibold tracking-wider text-primary uppercase">{co.short}</span>
                      <h3 className="font-heading font-semibold text-charcoal text-sm leading-snug">{co.name}</h3>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground leading-relaxed mb-4">{co.desc}</p>
      ...
      <section className="py-16 bg-surface-subtle border-b border-divider">
        <div className="container max-w-7xl mx-auto px-6">
          ...
          <h2 className="font-heading text-2xl text-charcoal mb-10">Manufacturing & Distribution Network</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {locations.map((loc) => (
              <div key={loc.city} className="flex gap-4 p-5 bg-card border border-border hover:border-primary/30 transition-colors duration-200">
                ...
                <div>
                  <span className="text-[10px] font-semibold tracking-wider uppercase text-primary">{loc.type}</span>
                  <div className="font-heading font-semibold text-charcoal text-sm mb-1">{loc.city}</div>
                  <p className="text-xs text-muted-foreground leading-relaxed">{loc.detail}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Timeline */}
      <section ref={ref2} className="fade-up py-16 border-b border-divider">
        <div className="container max-w-7xl mx-auto px-6">
          <p className="section-label mb-3">History</p>
          <h2 className="font-heading text-2xl text-charcoal mb-10">Company milestones</h2>
          <div className="relative">
            <div className="absolute left-20 top-0 bottom-0 w-px bg-divider hidden md:block" />
            <div className="space-y-6">
              {milestones.map((m) => (
                <div key={m.year} className="flex gap-8 items-start">
                  <div className="w-20 shrink-0 font-heading font-bold text-primary text-sm pt-0.5">{m.year}</div>
                  <div className="relative flex items-start gap-4">
                    <div className="hidden md:block w-2 h-2 bg-primary mt-1.5 shrink-0" />
                    <p className="text-sm text-charcoal-light">{m.event}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section ref={ref3} className="fade-up py-14 bg-surface-subtle">
        <div className="container max-w-7xl mx-auto px-6">
          ...
          <h2 className="font-heading text-2xl text-charcoal mb-4">Certifications & Standards</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={cert} className="border border-border bg-card p-4 text-center hover:border-primary/40 transition-colors duration-200">
                ...
                <div className="text-xs font-semibold text-charcoal">{cert}</div>
              </div>
            ))}
          </div>
        </div>
